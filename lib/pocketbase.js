...
export const getTodayCounts = async () => {
  try {
    ...
    // Try querying by 'created' field first
    ...
    // If no records found, try by 'date' field as fallback
    if (records.items.length === 0) {
      log("No records found by 'created', trying 'date' field");
      const dateQuery = `date >= "${dateRange.start}" && date < "${dateRange.end}"`;
      log("Date query filter", dateQuery);

      try {
        records = await pb.collection("mantras").getList(1, 500, {
          filter: dateQuery,
        });
        log(`Found ${records.totalItems} records by 'date' field`);
      } catch (error) {
        log("Error querying by 'date'", error);
        records = { items: [] };
      }
    }
    ...
    // Aggregate counts by mantra name - each record represents one count (IGNORE the count field)
    const mantraCounts = {};
    ...
    for (const record of records.items) {
      ...
      mantraCounts[name] += 1; // Each record counts as 1, ignore the record.count field
    }
...
export const createMantraEntry = async (mantraName) => {
  try {
    ...
    log("Current IST time for record", nowIST.toISOString());

    // Create new record with IST date - we don't need to set count since each record represents one count
    const data = {
      name: mantraName,
